...
interface ServiceProvidersByCategoryCrudProps {
  categoryId: string;
}

export function ServiceProvidersByCategoryCrud({
  categoryId,
}: ServiceProvidersByCategoryCrudProps) {
  ...
  const handleAddProvider = () => {
    router.push(`/dashboard/service-providers/add?categoryId=${categoryId}`);
  };

  const handleEditProvider = (provider: ServiceProvider) => {
    // Navigate to edit page with current search params preserved
    const currentParams = searchParams.toString();
    const editUrl = `/dashboard/service-providers/${categoryId}/edit/${provider.service_provider_id}${currentParams ? `?${currentParams}` : ''}`;
    router.push(editUrl);
  };
  ...
  const confirmDeleteProvider = async () => {
    ...
    try {
      ...
      setProviders(
        providers.filter(
          (p) => p.service_provider_id !== selectedProvider.service_provider_id
        )
      );
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will mark the service provider as deleted. This action cannot
              be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
