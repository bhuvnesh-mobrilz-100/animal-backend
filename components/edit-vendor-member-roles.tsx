...
export default function EditUserRolesModal({ 
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (user && selectedVendor) {
        getUserCurrentRoles(user.user_id, selectedVendor.vendor_id);
      }
    }
  }, [isOpen, user, selectedVendor]);
  ...
  const getUserCurrentRoles = async (userId: number, vendorId: number) => {
    setIsLoading(true);
    try {
      // Get all roles for the specific vendor (not just admin roles)
  ...
  const updateUserRoles = async () => {
    ...
    setIsUpdating(true);
    try {
      // First, get current roles for this vendor to compare
      ...
      // Remove roles that are no longer selected (for this vendor only)
      ...
      if (rolesToAdd.length > 0) {
        const newRoles = rolesToAdd.map(roleId => ({
          ...
          assigned_by: null,
          vendor_id: selectedVendor.vendor_id, // Set the vendor ID
  ...
  if (!user || !selectedVendor) return null;
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Edit Vendor Admin Roles
          </DialogTitle>
          <DialogDescription>
            Update admin roles for <span className="font-medium">{user.full_name}</span>
            <br />
            <span className="text-xs text-gray-500">{user.email}</span>
            <br />
            <span className="text-xs text-blue-600 font-medium">Vendor: {selectedVendor.name}</span>
          </DialogDescription>
        </DialogHeader>
            ...
            <>
              ...
              {userRoles.length === 0 && (
                <div className="text-sm text-amber-700 bg-amber-50 border border-amber-200 p-3 rounded-md">
                  ...
                  <p className="mt-1">User will have no roles for this vendor. They may lose access.</p>
                </div>
              )}
              ...
              {userRoles.length > 0 && (
                <div className="text-sm text-blue-700 bg-blue-50 border border-blue-200 p-3 rounded-md">
                  ...
                  <p className="mt-1">
                    {userRoles.length} role{userRoles.length !== 1 ? 's' : ''} will be assigned for {selectedVendor.name}.
                  </p>
                </div>
              )}
            </>
