...
import { deleteAnimalImageByUrl } from '@/lib/storage-upload';
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const normalizedImageUrl = normalizeImageUrl(updatePayload.image_url);
  if (normalizedImageUrl !== undefined) {
    updatePayload.image_url = normalizedImageUrl;
  }

  const normalizedAnimalTypeId = normalizeAnimalTypeId(updatePayload.animal_type_id);
  ...
  if (typeof previousImageUrl === 'string' && previousImageUrl && previousImageUrl !== normalizedImageUrl) {
    try {
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await supabaseAdmin.from('breeds').delete().eq('breed_id', params.id);
